...
#include "ui_dialogdinben.h"
#include "global.h"
#include "log.h"
DialogDinBen::DialogDinBen(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::DialogDinBen)
{
    ui->setupUi(this);
    ui->label3->setPixmap(QPixmap("://benbuxueshengshitang"));
    ...
    connect(act1_4,&QAction::triggered,this,[=]
    {
        ...
        Log log;
        QString thisLog = "";
        thisLog += "食堂导航   目的地：学生食堂一层(校区一)\n";
        log.print(thisLog);
        navi.wayPoint.push_back(currentSite);//将当前位置存入
        navi.wayPoint.push_back("学生食堂(校区一)");//将目的地存入
        currentSite = "学生食堂(校区一)";
        navi.selectStrategy();
        currentPoint = navi.route.pop();
        nextPoint = navi.route.pop();
    ...
    connect(act6_4,&QAction::triggered,this,[=]
    {
        ...
        Log log;
        QString thisLog = "";
        thisLog += "食堂导航   目的地：教工食堂一层(校区一)\n";
        log.print(thisLog);
        navi.wayPoint.push_back(currentSite);//将当前位置存入
        navi.wayPoint.push_back("教工食堂(校区一)");//将目的地存入
        currentSite = "教工食堂(校区一)";
        navi.selectStrategy();
        currentPoint = navi.route.pop();
        nextPoint = navi.route.pop();
        startNavi = true; //可以开始导航
